...
const StockItemForm: React.FC<StockItemFormProps> = ({
  ...
  // Requisition screens show generic formulary name, not brand
  ...
  const form = useForm<z.infer<typeof formSchema>>({
    ...
    defaultValues: {
      ...
      quantity: stockOperationItem.quantity,
    },
    mode: 'all',
  });
  ...
  // Brand/common name display — used for non-requisition operations
  ...
  // Generic formulary name — used for requisition operations only.
  // Shows the formulation without brand e.g. "Paracetamol 120 mg per 5ml Oral Suspension"
  // regardless of which brand was selected by the search.
  const genericName = useMemo(() => {
    // 1. Server-computed displayName (preferred — set by StockItemDTO.getDisplayName())
    if ((item as any)?.displayName) {
      return (item as any).displayName;
    }
    // 2. Split drugName on " - " and take the formulary (left) portion
    if (item?.drugName) {
      const sep = item.drugName.indexOf(' - ');
      return sep > -1 ? item.drugName.substring(0, sep).trim() : item.drugName;
    }
    // 3. commonName for concept-only stock items
    if (item?.commonName) {
  ...
  return (
    <Form onSubmit={form.handleSubmit(onSubmit)} className={styles.form}>
      <Stack gap={4} className={styles.grid}>
        {/* Requisition → generic formulary name; all others → brand/common name */}
        <p className={styles.title}>
          {isRequisitionOperation ? genericName : useItemCommonNameAsDisplay ? commonName : drugName}
        </p>
